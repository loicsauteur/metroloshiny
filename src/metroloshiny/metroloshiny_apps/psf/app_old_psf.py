    ...
    with ui.layout_sidebar():
        ...
        with ui.navset_card_underline(title="PSF over time"):
            with ui.nav_panel(title="Plot"):
                ...
                @render.data_frame
                def test():
                    """Before rewrite of new app, temporary test."""
                    create_fwhm_df()
                    return pd.DataFrame()
            ...
            with ui.nav_panel(title="Table"):
                ...
                @render.data_frame
                def show_psf_over_time_table():
                    """Show the PSF over time data table."""
...
@reactive.event(
    ...
    input.theoretical,
    theoretical_fwhm,
)  # should be:  input.ch_selection, input.fwhm_selection, input.theoretical, theoretical_fwhm, input.date_range_selection,
def create_fwhm_df() -> pd.DataFrame:
    """
    Create the final dataframe for plotting FWHM dataa.

    Filter the filtered dataframe by card selection for FWHM.
    Replaces create_display_df.
    ...
    # # Sanity check
    # if _df is None:
    #     raise RuntimeError("DataFrame was empty where it shouldn't!")
    # Return if dataframe is empty
    if _df is None or _df.empty:
        return _df

    # Remove non FWHM rows (i.e. when they start with FWHM)
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
            message="Boolean Series key will be reindexed to match DataFrame index",
        )
        _df = _df[df["FWHM"].str.startswith("FWHM")]
...
@reactive.calc
@reactive.event(df_data)  # should be: date_range_selection, ch_selection
def create_shift_df() -> pd.DataFrame:
    """
    Filter the filtered dataframe by card selection for shift.
...
def clean_dataframe() -> pd.DataFrame:
    ...
    Removes al NAN columns.
    Makes sure that values are numeric.
    Filters also by the selected date range.
...
# @reactive.effect
# def create_card_data_selectors_new():
#     """see above

#     FIXME multiple caveats...
#     use something like this to update the checkbox group, which does not work currently
#     TODO: rewrite make it more stream lined:
#     - reactive value for df filtered on side bar elements
#     - reactive calcs for updating card selections
#     - TODO: future, go away from channel names, use wavelengths directly! -> needs changing upload too... (try to get the ex wavelengths from the omero channels)
#     - TODO: implement getting info from objective IDs database
...
#     channels = np.unique(np.array(_df["Channel"]))
#     ch_dict = {ch: ch for ch in channels}
#     ch_check_boxes.set(ui.input_checkbox_group("ch_selection", "", choices=ch_dict, selected=list(ch_dict.keys())))
#     ui.update_select("ch_calc_selection", choices=list(channels))

#     fwhms = np.unique(np.asarray(_df["FWHM"]))
#     fwhm_dict = {f: f for f in fwhms}
#     fwhm_check_boxes.set(
#         ui.input_checkbox_group("fwhm_selection", "", choices=fwhm_dict, selected=list(fwhm_dict.keys()))
#     )
