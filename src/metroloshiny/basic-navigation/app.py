...
with ui.nav_panel("Page 1"):
    with ui.navset_card_underline(title="Penguins data", footer=footer):
        with ui.nav_panel("Plot"):
            ...
            def hist():
                p = sns.histplot(
                    penguins, x=input.var(), facecolor="#007bc2", edgecolor="white"
                )
...
# Load data
df_power_data = read_xlsx()
power_lines = ["All"]
power_lines.extend(np.unique(np.asarray(df_power_data[df_power_data.columns[0]])))
#power_prct = ["All"]
...
selection_line = ui.input_select("line", f'Select {df_power_data.columns[0]}', choices=list(power_lines))
selection_date = ui.input_select("date", "Select date", choices=measurement_dates)
selection_power = ui.input_select("power_prct", "Select the power %", choices=list(power_prct))


# Create a side bar, which is shown the same for all pages (per page not possible?)
#with ui.sidebar():
#    pass
...
with ui.nav_panel("Laser Power"):
    ...
    with ui.navset_card_underline(title="Power Linearity", header=[selection_date]):
        ...
        with ui.nav_panel("Plot"):
            ...
            def power_linearity_per_date():
                ...
                df = df_power_data[[df_power_data.columns[0], df_power_data.columns[1], input.date()]]
                #print("************")
                ...
                sns.lineplot(
                    ...
                    palette='turbo',
                    hue_norm=(380, 700),
                    legend="full",  # ensure that the poper line values are shown in the legend
                    #estimator=None,
    ...
    with ui.navset_card_underline(title="Power over time", header=[selection_line, selection_power]):
        ...
        with ui.nav_panel("Plot"):
            ...
            def power_over_time():
                ...
                prct = None if input.power_prct() == "All" else int(input.power_prct())

                df = get_power_over_time_data(df_power_data, line=line, power_prct=prct, merge=True)
                sns.lineplot(
                    df,
                    x="Date",
                    y="mW",
                    hue=df.columns[1],  # Do not merge values, use individual lines per row
                    palette='turbo',  # Colors
                    hue_norm=(380, 700),  # Color palette normalization on Line [nm]
                    legend="full",  # ensure that the poper line values are shown in the legend
                )
    ...
    # with ui.navset_card_underline(title="Power linearity"):#, footer=selection):
            
    #         with ui.nav_panel("Plot"):

    #             @render.plot
    #             def power_linearity_per_day():
    #                 # Select only the rows, which correspond to the selected power line (nm)
    ...
    #                 palette = {w: wavelength_to_color(w) for w in wavelengths}
    ...
    #                 # Needs header to be all string (all numbers does not work)
    #                 df = get_linearity(df_power_data, input.date())
    #                 return df
    #         #with ui.na
