...
with ui.navset_pill(id="tab"):
    ...
    with ui.nav_panel("Laser Power at Objective"):
        # Load Data
        gsheet, df = get_laser_power_objective_data(dev_local_file=False) # FIXME temp tests on local file
        ...
        def update_microscope_choices():
            # Filter the data frame (always the original) and set the reactive result dataframe
        ...
        def update_line_choices():
            ...
            ui.update_select("kind", choices=light_kinds.get(), selected=light_kinds.get()[0])


        # Update wavelength choices based on light source
        @reactive.effect
        @reactive.event(input.kind, input.info, input.objective, input.microscope)
        ...
        @reactive.effect
        @reactive.event(input.line, input.kind, input.info, input.objective, input.microscope)
        ...
        with ui.layout_sidebar():
            ...
            with ui.navset_card_underline(header=[single_date_selection]):
                ...
                with ui.nav_panel(title="Plot"):
                    ...
                    @reactive.effect
                    @reactive.event(input.date, input.line, input.kind, input.info, input.objective, input.microscope)
                    def get_power_linearity_table() -> pd.DataFrame:
                        # Filter (remove columns)
                        table = filter_by_column_value(df, "Site", input.site())
                        table = filter_by_column_value(table, "Microscope", input.microscope())
                        table = filter_by_column_value(table, "Objective", input.objective())
                        table = filter_by_column_value(table, "Info", input.info())
                        # drop the unused light-source column
                        col_to_drop = "Laser Line [nm]" if input.kind() == "LED Line [nm]" else "LED Line [nm]"
                        table = table.drop(columns=[col_to_drop])
                        # Filter by wavelength
                        if input.line() is not None and input.line() != "All":
                            table = filter_by_column_value(table, input.kind(), float(input.line()), drop_column=False)
                        ...
                        if input.kind() is not None and input.date() is not None:
                            table = table[[input.kind(), "Power [%]", input.date()]]
                    ...
                    def power_linearity_per_date():
                        ...
                        if table is None:
                            return
                        # Work on a copy of the table, otherwise the reactive value is changed
                        table = table.copy()
                        # Merge the frist two columns
                        table["Line [nm] @ [%]"] = table[table.columns[0]].astype(int).astype(str) + " @ " + table[table.columns[0]].astype(int).astype(str)
                        # Reorder columns
                        cols = list(table)
                ...
                with ui.nav_panel(title="Plot"):
                    ...
                    def get_power_stability_table():
                        ...
                        if input.line() is not None and input.line() != "All":
                            table = filter_by_column_value(table, input.kind(), float(input.line()), drop_column=False)
                        # Filter by power
                        if input.power() is not None and input.power() != "All":
                            table = filter_by_column_value(table, "Power [%]", float(input.power()), drop_column=False)
                    ...
                    def power_stability_over_time():
                        ...
                        table = pd.DataFrame(table) # FIXME - temp for datatype...
                        if table.empty:
                            return
