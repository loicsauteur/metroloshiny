...
from metroloshiny.utils.common_utils import is_input_select_in_list, get_ui_id
...
# Optional selections
# Common selectors
microscope_selector = ui.input_select("microscope", "Select a microscope", choices=[])
objective_selector = ui.input_select("objective", "Select an objective", choices=[])
info_selector = ui.input_select("info", "Filter by info column", choices=[])
new_mic_name = ui.input_text("new_mic_name", "Enter a name for a new Microscope entry", "Enter name...")
new_obj_name = ui.input_text("new_obj_name", "Enter a name for a new Objective", "Enter name...")
new_info_name = ui.input_text("new_info_name", "Enter additional information", "Enter info...")
...
with ui.nav_panel(title="Data Upload"):
    ...
    with ui.layout_sidebar():
        with ui.sidebar():
            ui.input_select("category", "Select a Metrology Categroy", choices=category_list, selected="PSF")
...
def update_on_cat_choice():
    ...
    if input.category() == category_list[0]:
        card_selectors.set([
            microscope_selector,  new_mic_name,
            objective_selector, new_obj_name,
            info_selector, new_info_name,
            kind_selector, line_selector, power_selector
        ])
    ...
    # PSF
        ...
        if isinstance(g_spreadsheet, pd.DataFrame):
            ...
            sheet = g_spreadsheet.worksheet("laser_power_objective_measurements")
            df = pd.DataFrame(sheet.get_all_records())
            dataframe.set(df)
    else:
...
def test_add_name_field():
    """
    TODO make new microscope name field only visible when "* New microscope *" selected
    ...
    # if card_selectors.get() and is_input_select_in_list(card_selectors.get(), "line"):
    #         print("card_selectors contains the line selector")
    # else:
    #     print("card_selectors does NOT contain the line selector")
        
    # if card_selectors.get() and any(x.id == "line" for x in card_selectors.get()):
